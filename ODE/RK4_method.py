...
After successful validation, RK4 method is performed manually.

No built-in ODE solver is used for RK4's actual calculation.
...
def rk4_method(
    ...
    # Safety limit against runaway RK4 solutions.
    max_abs_y = 1e100
    ...
    for i in range(number_of_steps):
        ...
        try:
            ...
            raise RuntimeError(
                f"RK4 failed at step {i}, "
                f"x={x_values[i]:g}, "
                f"y={y_values[i]:g}."
            ) from exc

        if not math.isfinite(k1) or not math.isfinite(k2) or not math.isfinite(k3) or not math.isfinite(k4):
        ...
        y_values[i + 1] = (
            y_values[i] + (1.0/6.0) * (k1 + 2.0*k2 + 2.0*k3 + k4) * h
        )
        ...
        if not math.isfinite(
            ...
            raise FloatingPointError(
                f"RK4 produced NaN/infinity at "
                f"x={x_values[i+1]:g}."
            )
...
def plot_reference_vs_rk4(
    ...
    plt.title(
        f"{ref_label} vs RK4 Method (h={comparison_h:g})"
    )
    ...
    plt.savefig(
        output / "02_reference_vs_rk4.png",
        dpi=300,
        bbox_inches="tight",
    )
...
def plot_different_step_sizes(
    ...
    plt.title(
        "RK4 Method for Different Step Sizes"
    )
...
def plot_step_size_effect(
    ...
    plt.title(
        "Effect of Step Size on RK4 Method"
    )
