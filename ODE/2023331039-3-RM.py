...
def ralston_method(
    ...
    """
    Handwritten Ralston's method (Runge-Kutta 2nd Order with a2=2/3).

        k1 = f(x_n, y_n)
        k2 = f(x_n + 0.75*h, y_n + 0.75*k1*h)
        y_(n+1) = y_n + (1.0/3.0*k1 + 2.0/3.0*k2)*h
...
def generate_results_table(
    ...
    """
    Generate summary table across all step sizes for Ralston's method.
    """
    ...
    for h in problem.step_sizes:
        x_values, y_values = ralston_method(
            rhs,
            problem.x0,
            problem.y0,
            problem.xf,
        ...
        rows.append(
            {
                "Step size, h": h,
                f"Ralston {dep}({xf:g})": numerical_final,
    ...
    table.to_csv(
        output / "ralston_summary_table.csv",
        index=False,
    )
    return table
...
def generate_iteration_table(
    ...
    for i in range(number_of_steps):
        k1 = float(rhs(x, y))
        k2 = float(rhs(x + 0.75 * h, y + 0.75 * k1 * h))
        x_next = x + h
        y_next = y + (1.0 / 3.0 * k1 + 2.0 / 3.0 * k2) * h
    ...
    table = pd.DataFrame(rows)
    table.to_csv(
        output / f"ralston_steps_h{h:g}.csv",
        index=False,
    )
    return table
...
def plot_reference_vs_ralston(
    ...
    plt.title(
        f"{ref_label} vs Ralston's Method (h={comparison_h:g})"
    )
...
def print_results(table: pd.DataFrame):
    title = "RALSTON'S METHOD SUMMARY TABLE"
    print(format_table_console(table, title))


def print_iteration_results(table: pd.DataFrame, h: float):
    title = f"RALSTON'S METHOD STEP-BY-STEP ITERATION TABLE (h = {h:g})"
    print(format_table_console(table, title))
