...
After successful validation, Heun's method is performed manually.

No built-in ODE solver is used for Heun's actual calculation.
...
def heun_method(
    ...
    """
    Handwritten Heun's method (Runge-Kutta 2nd Order with a2=1/2).
    ...
    # Safety limit against runaway Heun solutions.
    max_abs_y = 1e100
    ...
    for i in range(number_of_steps):
        ...
        if not math.isfinite(
            ...
            raise FloatingPointError(
                f"Heun produced NaN/infinity at "
                f"x={x_values[i+1]:g}."
            )

        if abs(y_values[i + 1]) > max_abs_y:
            raise FloatingPointError(
                f"Heun solution became unstable near "
                f"x={x_values[i+1]:g}."
            )
...
def generate_results_table(
    ...
    """
    Generate summary table across all step sizes for Heun's method.
    """
    ...
    table.to_csv(
        output / "heun_results.csv",
        index=False,
    )
    table.to_csv(
        output / "heun_summary_table.csv",
        index=False,
    )
    return table
...
def generate_iteration_table(
    ...
    for i in range(number_of_steps):
        k1 = float(rhs(x, y))
        k2 = float(rhs(x + h, y + k1 * h))
        x_next = x + h
        y_next = y + 0.5 * (k1 + k2) * h
...
def plot_reference_vs_heun(
    ...
    plt.savefig(
        output / "02_reference_vs_heun.png",
        dpi=300,
        bbox_inches="tight",
    )
...
def plot_different_step_sizes(
    ...
    plt.title(
        "Heun's Method for Different Step Sizes"
    )
...
def plot_step_size_effect(
    ...
    plt.title(
        "Effect of Step Size on Heun's Method"
    )
